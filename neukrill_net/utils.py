...
def write_output(fname, names, prob_matrix, settings, compress=True):
    """
    Utility function to write the output csv
    input: fname - of desired csv
           names - image names
           prob_matrix - predictions e.g. those generated by clf.predict_proba(X)
    """

    with open(fname, 'w') as csv_out_fh:
        writer = csv.writer(csv_out_fh, delimiter=',')
        writer.writerow(['image'] + settings.classes)
        for index in range(len(names)):
            writer.writerow([names[index]] + p[index,])

    if compress:
        compress_file(fname)


def compress_file(original_fname):
    """
    Utility to compress a file using gzip and then remove the original file
    input: original_fname
    """

    compressed_fname = fname + ".gz"
    with open(original_fname, 'rb') as original_fh:
        with gzip.open(compressed_fname, 'wb') as gzip_fh:
            gzip_fh.writelines(original_fh)

    # if compression has worked and the file exists then remove the
    # uncompressed file
